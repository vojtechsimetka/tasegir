...
> Fork of [tasegir](https://github.com/auhau/tasegir) specializing on TypeScript projects. 
...
with [some custom rules](https://github.com/ipfs/eslint-config-tasegir) to enforce some more strictness.
...
By default, `tasegir` uses `src/index.js` as the entry file for Webpack. You can customize which file to use as the entry point by specifying `entry` field in your user configuration file. To do this, create `.tasegir.js` file in your project's root diretory and add point the `entry` field to the file Webpack should use as the entry:
...
If `.tasegir.js` file is not present in the project, webpack will use `src/index.js` as the default entry file.
